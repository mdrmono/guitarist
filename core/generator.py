...
def build_chord_asset(requested: str) -> ChordAsset:
    ...
    return ChordAsset(
        ...
        audio_wav=generate_chord_wav(voicing),
        diagram_filename=f"{stem}.svg",
        audio_filename=f"{stem}.wav",
    )


def prepare_generation(input_text: str) -> PreparedGeneration:
    assets: List[ChordAsset] = []
    unsupported: List[UnsupportedInput] = []
    for requested in parse_chord_inputs(input_text):
        try:
            assets.append(build_chord_asset(requested))
